...
Logic in pseudo-code
    ...
    2 - For each run
        ...
        2.4 - For each fold
            ...
            2.4.4 - Write feature vectors in libsvm format
"""
...
def generate_feature_vectors(x_train, y_train, x_test, y_test, work_dir):
...
if  __name__ == '__main__':
    ...
    for run in range(1, args.repetitions+1):
        ...
        logging.info('\tFolding the dataset ...')
        ...
        for train, test in folds:
            ...
            generate_feature_vectors(x_train, y_train, x_test, y_test, fold_dir)
            logging.info(''.join(['\t\tFolding ', str(fold_id), ' finished.']))

        logging.info(''.join(['\tRun ', str(run), ' finished.']))
