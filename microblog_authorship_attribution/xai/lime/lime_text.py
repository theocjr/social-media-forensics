...
class IndexedCharGrams(object):
    """Character grams with various indexes."""

    def __init__(self, raw_string, ngrams_size=4, bow=True, mask_string=None):
        """Initializer.

        Args:
            raw_string: string with raw text in it
            num_grams: number of characters in each gram. Default = 4.
            bow: if True, a word is the same everywhere in the text - i.e. we
                 will index multiple occurrences of the same word. If False,
                 order matters, so that the same word will have different ids
                 according to position.
        ...
        self.as_list = [ ''.join(gram) for gram in ngrams(self.raw, self.ngrams_size) ]
        self.as_np = np.array(self.as_list)
        self.string_start = np.array(range(0, len(self.raw)-(self.ngrams_size-1) ))

        vocab = {}
        self.inverse_vocab = []
        self.positions = []
        self.bow = bow
        for i, gram in enumerate(self.as_np):
            if bow:
                if gram not in vocab:
                    vocab[gram] = len(vocab)
                    self.inverse_vocab.append(gram)
                    self.positions.append([])
                idx_gram = vocab[gram]
                self.positions[idx_gram].append(i)
            else:   # NOT IMPLEMENTED
                logging.error('bow=False not implemented!')
                return None
                #self.inverse_vocab.append(gram)
                #self.positions.append(i)
        if not bow: # NOT IMPLEMENTED
            logging.error('bow=False not implemented!')
            return None
            #self.positions = np.array(self.positions)
    ...
    def word(self, id_):
        """Returns the word/gram that corresponds to id_ (int)"""
        return self.inverse_vocab[id_]
    ...
    def inverse_removing(self, grams_to_remove):
        """Returns a string after removing the appropriate grams.

        If self.bow is false, replaces gram with UNKWORDZ instead of removing
        it.

        Args:
            grams_to_remove: list of ids (ints) to remove

        Returns:
            original raw string with appropriate words removed.
        """
        mask = np.ones(self.as_np.shape[0], dtype='bool')
        mask[self.__get_idxs(grams_to_remove)] = False
        if not self.bow:    # NOT IMPLEMENTED
            return ''.join(
                [self.as_list[i] if mask[i] else self.mask_string
                 for i in range(mask.shape[0])])
        grams = [ self.as_list[v] for v in mask.nonzero()[0] ]
        if len(grams) > 0:
            return grams[0][0:-1] + u''.join([ gram[-1] for gram in grams ])
        return u''

    def __get_idxs(self, grams):
        """Returns indexes to appropriate grams."""
        if self.bow:
            return list(itertools.chain.from_iterable(
                [self.positions[gram] for gram in grams]))
        else:   # NOT IMPLEMENTED
            return self.positions[grams]
...
class LimeTextExplainer(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            char_n_gram: if higher than 0, consider the input as character
                n-grams. This option is incompatible with char_level option.
        """
        ...
        if char_level and char_n_gram > 0:
            logging.error('char_level and char_n_grams are incompatible options.')
            return None
    ...
    def explain_instance(self,
        ...
        if self.char_n_gram:
            indexed_string = IndexedCharGrams(text_instance,
                                              ngrams_size=self.char_n_gram,
                                              bow=self.bow,
                                              mask_string=self.mask_string,
                                             )
        elif self.char_level:
            indexed_string = IndexedCharacters(text_instance,
                                               bow=self.bow,
                                               mask_string=self.mask_string,
                                              )
