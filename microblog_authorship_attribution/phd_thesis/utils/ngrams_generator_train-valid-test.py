...
def ngrams_generator(tweets, features, dest_dir, hapax):
    char_word_len = None
...
if  __name__ == '__main__':
    ...
    for author_dir in author_dirnames:
        ...
        author_dest_dir = os.sep.join([args.dest_dir, author_id, 'training'])
        os.makedirs(author_dest_dir)
        with open(os.sep.join([author_dir, 'training.json']), mode='rt', encoding='ascii') as fd:
            messages = json.load(fd)
        with open(os.sep.join([author_dir, 'valid.json']), mode='rt', encoding='ascii') as fd:
            messages += json.load(fd)
        ngrams_generator(messages, args.features, author_dest_dir, True)

        author_dest_dir = os.sep.join([args.dest_dir, author_id, 'test'])
        os.makedirs(author_dest_dir)
        with open(os.sep.join([author_dir, 'test.json']), mode='rt', encoding='ascii') as fd:
            messages = json.load(fd)
        ngrams_generator(messages, args.features, author_dest_dir, False)
