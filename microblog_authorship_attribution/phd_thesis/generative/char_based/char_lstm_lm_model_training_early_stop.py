...
def command_line_parsing():
    ...
    parser.add_argument('--highway_layers',
                        type=int,
                        default=2,
                        help='Number of layers in the highway network (stacked) (Default = 2).')
    ...
    parser.add_argument('--early_stop',
                        type=int,
                        default=0,
                        help='Number of epochs without improvement to stop the training early (Default = 0 -- no early stop).')
    ...
    parser.add_argument('--tag_url',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Tag URLs (Default = False).')
    parser.add_argument('--tag_user',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Tag user references (Default = False).')
    parser.add_argument('--tag_number',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Tag numbers (Default = False).')
    parser.add_argument('--tag_hashtag',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Tag hashtags (Default = False).')
    parser.add_argument('--demojize',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Transform emojis into a short textual description (Default = False).')
    parser.add_argument('--textify_emoji',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Transform emojis into a long textual description (requires --demojize) (Default = False).')
    parser.add_argument('--mention_limit',
                        type=int,
                        default=0,
                        help='Pre-processing option. Limits the consecutive user references repetitions (Default = 0).')
    parser.add_argument('--punc_limit',
                        type=int,
                        default=0,
                        help='Pre-processing option. Limits the consecutive punctuation repetitions (Default = 0).')
    parser.add_argument('--lower_hashtag',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Lower case hashtags (Default = False).')
    parser.add_argument('--segment_hashtag',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Segment hashtags (Default = False).')
    parser.add_argument('--lower_case',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Lower case text (Default = False).')
    parser.add_argument('--add_capital_signs',
                        action='store_true',
                        default=False,
                        help='Pre-processing option. Add capital letter tags (Default = False).')
    ...
    parser.add_argument('--save_optimizer',
                        action='store_true',
                        default=False,
                        help='Save the optimizer state in the checkpoint file to resume training (default = False).')
...
def initialize_weights(module):
    initialize_weights_uniform(module, -0.05, 0.05)


def initialize_weights_uniform(module, lower_bound, upper_bound):
    if isinstance(module, torch.nn.modules.sparse.Embedding):
        torch.nn.init.uniform_(module.weight.data, lower_bound, upper_bound)
    elif isinstance(module, torch.nn.modules.conv.Conv1d):
        torch.nn.init.uniform_(module.weight.data, lower_bound, upper_bound)
        torch.nn.init.uniform_(module.bias.data, lower_bound, upper_bound)
    elif isinstance(module, torch.nn.modules.linear.Linear):
        torch.nn.init.uniform_(module.weight.data, lower_bound, upper_bound)
        torch.nn.init.uniform_(module.bias.data, lower_bound, upper_bound)
    elif isinstance(module, torch.nn.modules.rnn.LSTM):
        for parameter in module.parameters():
            torch.nn.init.uniform_(parameter, lower_bound, upper_bound)
    else:
        logger.debug(f'Unregistered layer type: {type(module)} not ininitialized.')
...
if __name__ == '__main__':
    ...
    for author_id in author_ids:
        dataset.append(datasets.load_dataset('text', data_files={'training': os.path.join(args.input_dir, author_id, 'training_text.txt'),
    ...
    for dataset_idx in range(len(dataset)):
        dataset[dataset_idx] = dataset[dataset_idx].map(lambda data: {'preprocessed': preprocessor(data['text'])})
    ...
    word_tokenizer.train_from_iterator(iterator=itertools.chain(*[ dataset_author['training']['preprocessed'] for dataset_author in dataset ]),
                                       vocab_size=60_000,                          # maximum number of tokens in the final tokenizer
    ...
    logger.info(f'Word tokenizer built with {word_tokenizer.get_vocab_size()} tokens.')
    with open(os.path.join(args.output_dir, 'config_tokenizer.json'), mode='xt', encoding='utf-8') as fd:
        json.dump({'model_type': 'bert'}, fd)
    word_tokenizer.save_model(args.output_dir)
    logger.info(f'Word tokenizer saved at {args.input_dir}')

    word_tokenizer = transformers.BertTokenizer.from_pretrained(args.output_dir,
                                                                do_lower_case=False,
                                                                tokenize_chinese_chars=True,
                                                                strip_accents=False,
                                                               )
    logger.info(f'Tokenizer loaded. Size: {len(word_tokenizer)}')
    number_added_tokens = word_tokenizer.add_special_tokens({'additional_special_tokens': special_tokens})
    if number_added_tokens > 0:
        logger.info(f'{number_added_tokens} special tokens added to the vocabulary. New size: {len(word_tokenizer)}')

    logger.info('Tokenizing the dataset ...')
    for dataset_idx in range(len(dataset)):
        dataset[dataset_idx] = dataset[dataset_idx].map(lambda data: { 'tokenized': word_tokenizer.tokenize(data['preprocessed']) })

    logger.info('Encoding dataset ...')
    for dataset_idx in range(len(dataset)):
        dataset[dataset_idx] = dataset[dataset_idx].map(lambda data: word_tokenizer(data['preprocessed'],
                                                                                    max_length = 128,
                                                                                    truncation=True,
                                                                                    padding=False,
    ...
    for author_idx, author_id in enumerate(author_ids):
        ...
        char_vocab['[unk]'] = len(char_vocab)    # unknown marker
        for message in dataset[author_idx]['training']:
        ...
        for data_id, batches in [('training', training_batches),
            ...
            for sample in dataset[author_idx][data_id]:
            ...
            for batch in batches:
                ...
                for sample in batch['x']:
                    char_encoded_batch.append(char_encode_sample(sample,
                                                                 word_tokenizer,
                                                                 char_vocab,
                                                                 token_max_len+2,   # room for [sot] and [eot] markers
                                                                ))
                batch['x'] = torch.tensor(char_encoded_batch, dtype=torch.long)

        author_logger.info('Building the model ...')
        model = CharacterLanguageModel(char_vocabulary_size = len(char_vocab),
        ...
        author_logger.info('Initializing weights ...')
        with torch.no_grad():
        ...
        for epoch in range(1, args.max_epochs+1):
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(checkpoint_dict, os.path.join(author_dir, f'model_epoch-{best_epoch}.pt'))
                no_improvement = 0
            else:
                no_improvement += 1
        ...
        shutil.copy2(os.path.join(author_dir, f'model_epoch-{best_epoch}.pt'), best_model_filename)
