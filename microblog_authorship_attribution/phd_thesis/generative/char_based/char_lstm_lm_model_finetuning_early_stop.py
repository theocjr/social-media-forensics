...
if __name__ == '__main__':
    ...
    logger.info('Loading word vocabulary ...')
    special_tokens = ['<http>',
                      '<user>',
                      '<number>',
                      '<emoji>', '</emoji>',
    ...
    word_tokenizer = transformers.BertTokenizer.from_pretrained(args.model_dir,
                                                                do_lower_case=False,
                                                                tokenize_chinese_chars=True,
                                                                strip_accents=False,
                                                               )
    logger.info(f'Tokenizer loaded. Size: {len(word_tokenizer)}')
    special_tokens_filename = os.path.join(args.model_dir, 'special_tokens.json')
    ...
    for author_idx, author_id in enumerate(author_ids):
        ...
        pre_trained_checkpoint = torch.load(os.path.join(args.model_dir, 'best_model.pt'))
        # TODO recover these parameters from the JSON training file
        ...
        optimizer.load_state_dict(pre_trained_checkpoint['optimizer_state_dict'])
        best_valid_loss = pre_trained_checkpoint['validation_loss']
        model.to(device)
