...
def command_line_parsing():
    ...
    parser.add_argument('--output_classes_original',
                        required=True,
                        type=int,
                        help='Number of output classes in the original (bigger) network.')
...
if __name__ == '__main__':
    ...
    logging.info('\tBuilding original network ...')
    ...
    output_layer_original = networks.buildDCNNPaper(batch_size=config_params['batch_size'],
                                                    ...
                                                    output_classes=config_params['output_classes_original'],
                                                    padding='last'
                                                   )
    ...
    # Training objective
    l2_layers = []
    for layer in lasagne.layers.get_all_layers(output_layer_original):
    ...
    loss_train = lasagne.objectives.aggregate(
                                              lasagne.objectives.categorical_crossentropy(
                                                                                          lasagne.layers.get_output(output_layer_original, x_batch),
                                                                                          y_batch
                                                                                         ),
                                              mode='mean'
    ...
    all_params = lasagne.layers.get_all_params(output_layer_original)
    ...
    load_model(output_layer_original, config_params['parameters_filename'], updates, config_params['updates_filename'])


    logging.info('\tAdapting original network ...')
    output_layer = lasagne.layers.DenseLayer(
                                             output_layer_original.input_layer,
                                             num_units=config_params['output_classes'],
    ...
    for epoch in range(1, config_params['n_epochs']+1):
        ...
        if valid_accuracy > best_validation_accuracy:
            logging.debug('\t\tBetter validation accuracy found at epoch {}/{}: {} %'.format(epoch, config_params['n_epochs'], valid_accuracy))
