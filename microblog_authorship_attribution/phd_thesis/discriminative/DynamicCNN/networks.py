...
def buildDCNNPaperAddConv(batch_size,
                          vocab_size,
                          embeddings_size=48,
                          filter_sizes=[10,7,7],
                          nr_of_filters=[6,12,12],
    ...
    l_fold1 = DCNN.folding.FoldingLayer(l_conv1)
    l_pool1 = DCNN.pooling.DynamicKMaxPoolLayer(l_fold1,ktop,nroflayers=3,layernr=1)
    l_nonlinear1 = lasagne.layers.NonlinearityLayer(l_pool1,nonlinearity=parseActivation(activations[0]))
    ...
    l_fold2 = DCNN.folding.FoldingLayer(l_conv2)
    l_pool2 = DCNN.pooling.DynamicKMaxPoolLayer(l_fold2,ktop,nroflayers=3,layernr=2)
    l_nonlinear2 = lasagne.layers.NonlinearityLayer(l_pool2,nonlinearity=parseActivation(activations[1]))

    l_conv3 = DCNN.convolutions.Conv1DLayerSplitted(
        l_nonlinear2,
        nr_of_filters[2],
        filter_sizes[2],
        nonlinearity=lasagne.nonlinearities.linear,
    ...
    l_fold3 = DCNN.folding.FoldingLayer(l_conv3)
    l_pool3 = DCNN.pooling.KMaxPoolLayer(l_fold3,ktop)
    l_nonlinear3 = lasagne.layers.NonlinearityLayer(l_pool3,nonlinearity=parseActivation(activations[2]))

    l_dropout3=lasagne.layers.DropoutLayer(l_nonlinear3,p=dropout)
...
def buildDCNNPaperAddDense(batch_size,
                           vocab_size,
                           embeddings_size=48,
                           filter_sizes=[10,7],
                           nr_of_filters=[6,12],
    ...
    l_dense1 = lasagne.layers.DenseLayer(
        l_dropout2,
        num_units=output_classes*2,
        nonlinearity=parseActivation(activations[2])
        )
...
def buildDCNNPaperAddConvDense(batch_size,
                               vocab_size,
                               embeddings_size=48,
                               filter_sizes=[10,7,7],
                               nr_of_filters=[6,12,12],
    ...
    l_dense1 = lasagne.layers.DenseLayer(
        l_dropout3,
        num_units=output_classes*2,
        nonlinearity=parseActivation(activations[3])
        )
